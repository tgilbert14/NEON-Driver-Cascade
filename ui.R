      ...
      nav_panel(title = tagList(bs_icon("search"), " Search"), value = "search",
        div(class = "tab-head", div(class = "tab-head-text",
          h4("Search the network",
             info_pop("Searching the cascade",
        ...
        div(class = "search-modeswitch",
          radioButtons("searchMode", label = NULL, inline = TRUE,
            choices = c("Find a link" = "link", "Cascade strength" = "strength"),
            selected = "link")),

        # (a) FIND A LINK ----------------------------------------------------
        conditionalPanel(condition = "input.searchMode == 'link'",
          card(card_head("link-45deg", "Find a driver→response link across the network"),
            div(class = "search-controls",
              selectizeInput("searchLink", label = tagList(bs_icon("diagram-2"), " Pick a predicted link"),
              ...
              checkboxInput("searchSignifOnly", "Only sites where it tests significant (expected, n≥6, p<0.05)", value = FALSE)),
            uiOutput("searchLinkSummary"),
            div(style = "width:100%", DT::DTOutput("searchLinkTable")),
            p(class = "qc-cap-note", bs_icon("info-circle"),
              HTML(" Each row is this link tested at one site: <b>r</b> is within-site, <b>n</b> is overlapping years, <b>p</b> is a permutation p (a lower bound on autocorrelated annual series). “Expected” marks the biome where the mechanism is predicted; faded sites still ran it as out-of-biome corroboration. A single site's short series can't settle the link; the pooled test on Across NEON can.")))),

        # (b) CASCADE STRENGTH ----------------------------------------------
        conditionalPanel(condition = "input.searchMode == 'strength'",
          card(card_head("bar-chart-line-fill", "Rank sites by cascade strength"),
            div(class = "search-controls",
        ...
        p(class = "search-foot-caveat", bs_icon("exclamation-triangle"),
