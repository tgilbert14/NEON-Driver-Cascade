...
- **Georeference:** the scene origin (0,0) is the surveyed cloud's bbox-centre UTM
  **E=515527.413, N=3530802.807** (zone 12N) — exactly how `build_plot.py` recentres the plants
  (`scene_x = easting − originE`, `scene_z = northing − originN`; east = +X, north = +Z). A 64 m
  square is cropped north-up/east-right and draped on a `PlaneGeometry(64,64)` with `tex.flipY=false`.
- **Validation:** the CHM was sampled at each plant's computed UTM position; plant positions land on
  dark shrubs in the photo (mean brightness 57 under plants vs 115 over the whole crop) and the dry
  wash sits on the east side — confirming the transform. (1 m CHM barely resolves sparse desert
  shrub, so the height correlation is weak; the photo overlay is the strong check.)
