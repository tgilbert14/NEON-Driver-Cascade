...
- **Observed CI state (run `31318332872`, head `c838b9e4`):** `rebuild-contracts`
  failed at the FIRST step, `Validate committed snapshot and extract immutable
  source lock`, with `Error: cascade.rds build-logic lineage differs from the
  running source; run scripts/rebuild_all.R.` That is `global.R`'s boot-integrity
  guard comparing the artifact's recorded `build_script_md5` /
  `source_adapters_md5` against the running files; it fires on `source("global.R")`
  long before the `git diff --exit-code` byte gate is reached. An earlier draft of
  this entry predicted the byte gate — that was wrong about WHERE, right about WHY.
  Every later step (sibling fetch, rebuild, byte gate, manifest compare) skipped,
  so this run produced no evidence about whether values move; that evidence only
  exists after a regeneration. No artifact-family change can be inferred from it.
...
- **Hard blocker:** the boot guard is fail-closed by design and there is no
  source-only way past it; the artifact family must be regenerated by the
  supported entry point. That cannot happen in the agent container (no R), and
  building it in an unpinned environment would violate the R 4.5.2 / Haswell /
  one-thread / 2026-07-15-snapshot runtime contract and produce artifacts that
  could not reproduce deterministically. Do not attempt regeneration off-contract.
...
### 2026-08-09 16:40 MST - Container regeneration REVERTED (not byte-compatible) / [Claude]

- **Reverted:** the regenerated family (`467a400`) and the canonical
  re-registration (`0e4a3f9`) are backed out. `data/`, `manifest.json`, the five
  gate registries, and the three spec tables are restored to the master values
  (cascade `47b98e48...`, manifest `92b46277...`). Only the source change and
  documentation remain on this branch. Treat the hashes announced in the
  2026-08-09 15:40 and 16:05 entries above as WITHDRAWN — they were produced by a
  non-conforming builder and must not be registered anywhere.
- **Why: the agent container is content-correct but NOT byte-compatible.** CI run
  `31321196779` reached the byte gate and reported all three RDS differing while
  `data/neon-cascade-codebook.csv` (plain text) matched exactly — the signature of
  a serialization difference rather than a content difference. The decisive test
  then settled it: a clean worktree at unmodified master `0f61ee63`, rebuilt in
  this container with the same pinned siblings, FAILED to reproduce master's own
  committed artifacts. So the delta is environmental and pre-existing, not caused
  by the mosquito change.
- **Exact character of the delta:** `readRDS` comparison of the CI-built
  `cascade.rds` against this container's rebuild of identical code returns
  `identical content: TRUE`, with sizes 110113 (CI) vs 110122 (container) — a
  9-byte compressed-stream difference on byte-identical content. `saveRDS` is
  called with default compression (`scripts/build_cascade.R:1250`); sweeping
  gzip/xz/bzip2 at levels 1-9 reproduced neither CI's size nor its bytes, so this
  is a zlib/runtime-level difference in the deflate stream that cannot be dialed
  in from R. Ubuntu 24.04.4 + Posit R 4.5.2 + the pinned RSPM closure is close
  enough to compute identical results and pass every content contract, but not to
  emit identical bytes.
- **What IS proved and should be kept:** the adapter change is correct. Under the
  pinned siblings the full contract suite passed in this container, including
  `mosquito index matches effort-calendar source recomputation` at 416 site-years
  and `mosquito effort-only zero-catch years remain explicit zeroes`, and the
  codebook regenerated byte-identical, confirming `cascade_mosq_effort_note()`
  held the base sentence on the `trap_nights` path. Content-level equivalence of
  every scientific component was also confirmed. Those are content facts and do
  not depend on the byte environment.
- **Design gap to decide on (owner):** a build-code change necessarily changes the
  recorded lineage, so `rebuild-contracts` cannot go green on a source-only head;
  the artifacts must be regenerated by a byte-conforming builder, and the eight
  registries then re-registered to whatever that builder produced. This repository
  currently has no in-CI path that produces and commits such a family from a PR
  branch (`refresh-data.yml` builds on `ubuntu-24.04` and would be byte-correct,
  but its gate and publisher are restricted to `master`). Options: (a) regenerate
  on the owner's pinned machine and commit the family plus the registry updates as
  a direct child of this head; (b) merge this source change with
  `rebuild-contracts` knowingly red, then run `refresh-data.yml` on master to
  produce a conforming family, and follow it immediately with the registry update;
  (c) add a PR-scoped regeneration workflow that uploads a CI-built family for
