...
| Suite Living Poster | concise companion first impression with executable absence checks | NONE / NO DRIVER BYTE CHANGE (suite-platform) | Mosquito compact-cover PR #7 / merge `ec0f2ba` proved that correct art, CTA, and semantic markers can coexist with a surplus second headline, methods lead, three truth cards, and a CAN/CANNOT panel; the suite-wide 2026-08-05 cover cleanup further removes ornamental illustration disclaimers while retaining meaningful alt text, durable provenance, and scientific claim limits | no Driver artifact or ecological change; keep full method and claim detail inside the product and durable science package while the cover preserves only the load-bearing scientific limit in a compact disclosure | require one poster, one CTA, one Driver route, and one compact honesty disclosure; prohibit metric/method/truth layers, duplicate bridges/directories, and cover-visible art disclaimers; visually verify artwork-first narrow order and first-viewport CTA visibility where geometry permits |
