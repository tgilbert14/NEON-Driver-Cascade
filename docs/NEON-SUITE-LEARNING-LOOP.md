...
Driver baseline
  ...
  -> Phenology Seal 2 [EXACT LEGACY-PIN PARITY NEXT; CURRENT RELEASE SEALED]
...
| 11 | Driver v2 reintegration | **GATE 0 COMPLETE / PHENOLOGY SEAL 1 PUBLISHED AND VERIFIED — SIGNAL CHANGES HELD / NO CANONICAL BYTE CHANGE** | [`docs/DRIVER-V2-SYNTHESIS.md`](DRIVER-V2-SYNTHESIS.md) and the machine-readable compatibility ledger measure all nine current release families without executing sibling code; [`docs/PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the effect-blind contract; Seal 1 adds only the Driver-owned pure two-clock adapter, response-only interval model, one-way values-free climate mask, 38 synthetic fixture families, and an exact Ubuntu/R/package/BLAS runtime lock; PR #55 exact head `35c2ca7` merged as `6cb1e9e8` | nine packages remain reconciled; exact-head run `30977899036` and merged-default run `30978285438` passed literal-revision guards, all 38 fixtures across six fresh processes with byte-identical output, independent adversarial review, effect/climate isolation, exact Driver-calendar topology, canonical artifact reproduction, and generated-artifact nonmutation; the isolated Seal-1 jobs deserialized neither current nor legacy Phenology RDS, while the separate unchanged v1 rebuild used its recorded legacy pins solely to prove exact canonical reproduction | no blind repin and no newly eligible vote; both legacy green-up screens remain HOLD for v2 re-authorization, exact current v2 compatibility/model support remains `UNMEASURED`, and every v2 effect remains sealed | Pages run `30978284811` deployed merge `6cb1e9e8` and the public root returned HTTP 200; Driver impact `NONE`; next: run Seal 2 only against exact legacy pin `81e339e9...` and prove registered parity without inspecting current response/effect output; only a separately reviewed successful Seal 2 may advance to Seal 3A current response support |
...
|---|---|---|---|---|---|
| Driver baseline | temperature -> green-up | HOLD FOR V2 RE-AUTHORIZATION / PHENOLOGY SEAL 1 PUBLISHED / V1 ARTIFACT UNCHANGED | the published annual-temperature family still contains 18 eligible sites and 15/18 direction matches; the spring screen contains 8/18, but both were built from the legacy Phenology pin; the effect-blind [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the current-release response/effect contract, and its pure Seal-1 implementation plus exact `survival 3.8-9` / `metafor 5.0-1` runtime lock passed 38 synthetic fixtures across six clean processes; PR #55 head `35c2ca7`, merge `6cb1e9e8`, default run `30978285438`, and Pages `30978284811` publish that evidence; the isolated Seal-1 path accessed no current or legacy response family, while the separate unchanged v1 rebuild only proved canonical reproduction | preserve the exact v1 lineage; create no new vote or canonical byte; keep current compatibility/model support `UNMEASURED` and all v2 effects sealed | prove sections 8.3 and 9.3 only against exact legacy pin `81e339e9...` in Seal 2; do not inspect current response/effect output to repair parity, and do not begin Seal 3 until the separately reviewed parity gate passes |
...
| Plant Phenology Explorer | plant-year green-up onset, leaf-active duration, and green-up coverage/support | HOLD / SEAL 1 PUBLISHED / NO BYTE CHANGE | the previously disclosed current release contributes 346/346 direct Driver-calendar matches across 45 sites, with 39 sites at >=6 app-supported years; its data tree differs from the legacy Driver pin; Seal 1 implements the frozen registry as three pure, effect-locked modules and verifies 38 synthetic fixture families, exact calendar topology, deterministic replay, artifact nonmutation, and exact-head/default/Pages publication under PR #55 and merge `6cb1e9e8`, while current v2 compatibility/model support remains `UNMEASURED` | keep v1 artifacts unchanged; retain source-clock `thin_greenup` only as leaf-active context; use the independent date-clock interval response for v2; the isolated Seal-1 path accessed neither current/legacy Phenology RDS values nor a v2 effect path; the separate unchanged v1 rebuild used recorded legacy pins only for canonical reproduction | next: exact legacy-pin parity only in Seal 2 under [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md); current response-support access remains forbidden until a separately reviewed Seal 2 passes |
...
adapter/model/mask implementation is now synthetically verified under the exact
runtime lock. No current or legacy Phenology response family was deserialized and
no effect path ran. The next stage is exact legacy-pin parity in Seal 2, while
discharge feasibility evidence may be acquired without building a new app.
