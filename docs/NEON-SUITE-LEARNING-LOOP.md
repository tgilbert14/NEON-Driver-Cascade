...
published evidence—until their pinned CI, artifact, review, Pages, Connect, and
semantic-health gates close. Ground Beetle Pass 5 then completed its evidence-only
baseline at candidate `e19996d` and moved into release/science foundation work.
...
- Ground Beetle Tracker's earlier Posit `Startup Error` is stale: on 2026-07-22 the
  public app opened a Shiny session and rendered the 46-site picker. Release health
  still fails closed because no semantic-ready marker exists, a zero-argument
  custom-handler error reaches the client, the manifest drifts, and the effort
  denominator is catch-conditioned. Small Mammal's baseline outage is historical:
  Pass 1 restored it, and Cover V5 / Suite Living Poster V1 is production-verified
  through PR #86 / merge `c4c46fce` and Connect #125.
...
| 0A | Suite release-health preflight | IN PROGRESS — 3/3 STARTUPS RESTORED; BEETLE RELEASE UNSAFE | 2026-07-18 baseline plus pass-specific production checks found drift in all nine companion manifests, moving release inputs, five apps with no executable tests, and Startup Error states in Small Mammal, Phenology, and Ground Beetle. Small Mammal, Phenology, Plant Diversity, and Vegetation have validated pinned read-only CI, restricted refresh candidates, semantic health, exact manifests, and app-local governance. Fresh 2026-07-22 Beetle evidence shows a live session/picker but no semantic marker, a handler error, eight manifest mismatches, and catch-conditioned effort | revamp plan + four published app-local packages; Ground Beetle baseline candidate `e19996d`; Vegetation docs PR #9 merge `3391e70` and receipt `da466ea` | `NONE` (suite-platform); reuse the validated release shape in later passes, but keep product science app-specific | All three baseline startup errors now visibly start; Beetle still lacks a clean semantic release receipt; the other four pending companions have startup-only evidence |
...
| 5 | Ground Beetle Tracker | STARTUP RESTORED / RELEASE UNSAFE / PASS 5 FOUNDATION ACTIVE | baseline `ff82104`; live session and 46-site picker observed 2026-07-22; 8 manifest mismatches; zero-argument handler error; no semantic marker; no executable tests | governance, science contract, handoff, and knowledge package candidate `e19996d` | `CONTEXT / HOLD METRIC ADOPTION / NO DRIVER BYTE CHANGE`; effort is catch-conditioned until zero-carabid field opportunities are emitted | fresh first paint/session passed 2026-07-22, but semantic release verification remains open |
...
| Ground Beetle Tracker | expert-reconciled Carabidae activity-density, composition, introduced-species context, trend, and exploratory environmental scan | CONTEXT / HOLD METRIC ADOPTION / NO DRIVER BYTE CHANGE | baseline `ff82104` source preserves expert-ID/rank gates, introduced-species context, small-series guards, and circular-shift null, but builds effort only after selecting positive catch rows; a non-missing effort column on those rows does not prove zero-carabid opportunities are represented | no Driver artifact change; retain only companion context and the independent Driver resolver | implement and fixture an opportunity-complete field-effort table, rebuild/validate every site bundle and index, align all labels/exports, then require pinned manifest/offline/semantic/public evidence before reconsideration |
...
then opened Ground Beetle Pass 5. The evidence-only baseline is recorded at
`e19996d`; release/science foundation work is now active while metric adoption
remains held. The two cover candidates remain unpublished until their own gates
close.
