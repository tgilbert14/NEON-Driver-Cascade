...
Driver baseline
  ...
  -> Driver v2 integration gate [NO BLIND REPIN; PHENOLOGY SEAL 0 FROZEN]
  -> Phenology Seal 1 [PURE ADAPTER + SYNTHETIC CONTRACTS NEXT]
...
| 11 | Driver v2 reintegration | **GATE 0 COMPLETE / PHENOLOGY SEAL 0 FROZEN — SIGNAL CHANGES HELD / NO CANONICAL BYTE CHANGE** | [`docs/DRIVER-V2-SYNTHESIS.md`](DRIVER-V2-SYNTHESIS.md) and the machine-readable compatibility ledger measure all nine current release families without executing sibling code; [`docs/PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the effect-blind two-clock adapter, interval response model, support-mask, effect, pooling, and seal sequence before current response-support access | nine packages reconciled; two decision axes separate reusable-contract adoption from ecological-signal adoption; Phenology Seal 0 adds exact source, censoring, roster/plot, fit, feasibility, and no-look contracts | no blind repin and no newly eligible vote; both legacy green-up screens remain HOLD for v2 re-authorization, and exact current v2 compatibility/model support remains `UNMEASURED` | next: implement Seal 1 only—pure adapter, response-only model, values-free climate mask, synthetic fixtures, and exact runtime pin—then independently review and pass Seal 2 legacy parity before Seal 3 current support; acquire discharge evidence without building an app |
...
|---|---|---|---|---|---|
| Driver baseline | temperature -> green-up | HOLD FOR V2 RE-AUTHORIZATION / PHENOLOGY SEAL 0 FROZEN / V1 ARTIFACT UNCHANGED | the published annual-temperature family still contains 18 eligible sites and 15/18 direction matches; the spring screen contains 8/18, but both were built from the legacy Phenology pin; the effect-blind [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) now freezes the current-release response and effect contract without inspecting a current v2 response or effect | preserve the exact v1 lineage; create no new vote or canonical byte; keep current compatibility/model support `UNMEASURED` until the pure implementation is independently tested and merged | implement Seal 1 and pin exact `survival`/`metafor` versions plus the full runtime inventory; then prove legacy contracts in Seal 2 before the response-support, climate-mask, decision, and explicit-unseal sequence |
...
| Plant Phenology Explorer | plant-year green-up onset, leaf-active duration, and green-up coverage/support | HOLD / SEAL 0 FROZEN / NO BYTE CHANGE | the current release contributes 346/346 direct Driver-calendar matches across 45 sites, with 39 sites at >=6 app-supported years; its data tree differs from the legacy Driver pin; the sealed registry discloses exact source-shape facts but leaves current v2 compatibility/model support `UNMEASURED` | keep v1 artifacts unchanged; retain source-clock `thin_greenup` only as leaf-active context; use the independent date-clock interval response for v2, with no current response/effect access at Seal 0 | next implementation: pure Driver-owned adapter, response-only interval model, values-free climate-support mask, adversarial fixtures, exact runtime pin, and legacy parity under [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) |
...
0. Phenology Seal 0 is now frozen in
`docs/PHENOLOGY-V2-ADAPTER-SPEC.md`; the next implementation step is its pure,
effect-locked Seal-1 adapter/model/mask and synthetic contract, while discharge
feasibility evidence is acquired without building a new app.
