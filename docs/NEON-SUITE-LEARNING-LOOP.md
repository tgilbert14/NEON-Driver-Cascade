...
merge `89caa4359019d95fe99dd9d916163ec269b6572e`. Pass 6, Mosquito Pulse, is next.
...
| 0A | Suite release-health preflight | IN PROGRESS — 3/3 STARTUPS RESTORED; PASSES 1–5 RELEASE-SAFE | 2026-07-18 baseline found drift in all nine companion manifests, moving release inputs, five apps with no executable tests, and Startup Error states in Small Mammal, Phenology, and Ground Beetle. Pass-specific releases now give Small Mammal, Phenology, Plant Diversity, Vegetation, and Ground Beetle pinned validation, restricted refresh candidates, semantic health, exact manifests, and app-local governance. Ground Beetle final merge `89caa435` passed 46-bundle opportunity/index/manifest contracts plus exact production smoke | revamp plan + five published app-local packages; Vegetation docs authority `3391e70` / receipt `da466ea`; Ground Beetle release merge `89caa435` | `NONE` (suite-platform); reuse the validated release shape in later passes, but keep product science app-specific | All three baseline startup errors are closed; passes 1–5 have content-aware production receipts; Mosquito, Birds, Water Chemistry, and Inverts retain startup-only baseline evidence until their passes |
...
Passes 1–5 are complete and production-verified. Phenology Living Poster merge
...
Mosquito Pulse Pass 6 is the next one-app cycle.
