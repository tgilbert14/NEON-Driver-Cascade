...
Driver baseline
  ...
  -> Discharge F0 [PASSED; RECEIPT PUBLISHED; F1 METADATA ONLY; F2 UNAUTHORIZED]
...
Current authority stops at F1's authenticated values-free file inventory and
reviewed non-observation schema metadata; F2 remains unauthorized, and discharge
...
| Driver baseline | aquatic climate bridge | DEFER BUILD / DISCHARGE F0 PASSED / RECEIPT PUBLISHED / F1 METADATA ONLY | released Water and Inverts have zero exact terrestrial Driver site-code matches; domain-year proxies are descriptive diagnostics, not integration keys; F0 defines only response-anchored discharge-site-year and released QC-pass record presence, preserves all `FALSE` rows, and changes no Driver byte | no Driver artifact, adapter, `ann_flow()`, prior, or vote | run only the authenticated values-free file inventory and reviewed non-observation schema metadata; F2 and every support conclusion remain unauthorized pending an independently reviewed exact F1 receipt; reopen review only if a later F2 finds >=3 unambiguous streams each with >=6 common primary years |
...
| Complementary-app gap audit | streamflow / discharge `DP4.00130.001` as the aquatic hinge | DEFER BUILD / F0 PASSED / RECEIPT PUBLISHED / F1 METADATA ONLY / F2 UNAUTHORIZED | exact Inverts `ff23e994` / `data/sites` tree `080534ca` is represented only by committed ledger blob `c2aefd1a...` / SHA-256 `79bb4591...`: 210 density-eligible UTC-year stream cells across 24 sites, 23 with >=6 years; F0/F1/F2 never fetch or deserialize its RDS blobs; discharge support remains `UNMEASURED`; F0 freezes total response-anchored `FALSE`, `historical_uncorrected_1_min_only`, ordinary/BIGC chronology, pre-QC TOMB rejection, `namedLocation` identity, and `REOPEN_REVIEW`/`HOLD` mappings; PR #59 merged as `28f00ece`; receipt PR #60 head `8954c1f` merged as `b75996a` after exact CI, merged-master CI, Pages, and HTTP 200 | no `ann_flow()`, discharge summary, prior, app, effect, vote, or Driver byte | inventory only the authorized F1 file and non-observation schema metadata, then independently review its exact receipt before any F2 payload measurement; reopen review—not build—only after a later F2 measures >=3 unambiguous exact streams x >=6 common primary years |
