...
or Driver byte changed. The Breeding Birds Pass 7 scientific/product
reconciliation is now drafted against the 47-site candidate contract, but the
official Run 14, pull-request, merge, Pages, and Connect receipts remain
**PENDING**. This register therefore does not yet claim a production-verified
Birds release. Water Chemistry is the next app after those closeout identities are
...
| 0A | Suite release-health preflight | IN PROGRESS — 3/3 STARTUPS RESTORED; PASSES 1–6 RELEASE-SAFE; PASS 7 RECEIPTS PENDING | 2026-07-18 baseline found drift in all nine companion manifests, moving release inputs, five apps with no executable tests, and Startup Error states in Small Mammal, Phenology, and Ground Beetle. Pass-specific releases now give Small Mammal, Phenology, Plant Diversity, Vegetation, Ground Beetle, and Mosquito pinned validation, restricted refresh candidates, semantic health, exact manifests, and app-local governance. The Birds candidate contract now covers the complete 47-site opportunity/index/search/export family, but official Run 14 and publication receipts have not yet been supplied | revamp plan + six published app-local packages plus one Birds reconciliation draft; Vegetation docs authority `3391e70` / receipt `da466ea`; Ground Beetle release merge `89caa435`; Mosquito science/runtime receipt `91b4c71`, compact Pages authority `ec0f2ba`, and cover receipt `6450f01`; Birds official identities `PENDING` | `NONE` (suite-platform); reuse the validated release shape in later passes, but keep product science app-specific | All three baseline startup errors are closed; passes 1–6 have content-aware production receipts; Birds Run 14/PR/merge/Pages/Connect receipts are `PENDING`; Water Chemistry and Inverts retain startup-only baseline evidence until their passes |
...
| 7 | Breeding Birds | PASS 7 RECONCILIATION DRAFT — OFFICIAL RECEIPTS PENDING | candidate contract retains all 47 site rows across bird, Search-site, and cross-site export surfaces; 26,365 valid physical counts include 117 supported-zero counts, and 24,509 supported point-years include 79 supported-zero point-years; the exact comparison window is 2017–2024 and the common rarefaction target is 90 valid physical counts; temperature source exists for 47/47 sites, while complete realized-month support is 45/47 because BARR and TOOL each have 1/2 months and retain `NA` with no imputation | opportunity-complete physical-count and point-year grains; positive/supported-zero/unavailable separation; parent/subspecies collapse; flyover/method/distance support; shared bird/Search/export window and support schema; source-presence versus estimand-completeness boundary; exact claim limits drafted | `CONTEXT / HOLD DRIVER INGESTION / NO DRIVER BYTE CHANGE`; rarefied community context and birds-per-count remain descriptive and cannot support causal, abundance, population, or occupancy claims | **PENDING:** final Run 14 producer/validator/publisher and artifact receipts; candidate PR/head CI; merge/default-branch verification; Pages and Connect identities, semantic smoke, responsive/accessibility, and public interaction receipts |
...
| Breeding Birds | opportunity-complete breeding-bird community context at physical-count and point-year grains | CONTEXT / HOLD DRIVER INGESTION / NO DRIVER BYTE CHANGE | the Pass 7 candidate contract retains all 47 bird/Search-site/export rows, 26,365 valid physical counts with 117 supported-zero counts, and 24,509 supported point-years with 79 supported-zero point-years; the exact 2017–2024 comparison uses a 90-count rarefaction target; temperature source exists 47/47, but complete realized-month support is 45/47, with BARR and TOOL each 1/2 and `NA` rather than imputation | keep Driver artifacts unchanged; preserve source presence, realized-window support, opportunity outcome, analysis window, rarefaction target, and claim-limit fields through any future independent adapter; do not create a causal or abundance/population/occupancy vote | official Run 14, PR, merge, Pages, Connect, and Driver docs-closeout receipts remain `PENDING`; after those close, reconsider ingestion only at suite synthesis with the pinned released source, measured Driver joins/support, a registered mechanism, and old/new parity |
...
bytes remain unchanged. Breeding Birds Pass 7 now has a complete reconciliation
draft for its opportunity, support, fixed-window, rarefaction, export, and claim
contracts, but its official Run 14, PR, merge, Pages, and Connect receipts remain
`PENDING`; no production closeout is claimed. Water Chemistry Pass 8 is next after
those Birds receipt placeholders are replaced with exact evidence.
