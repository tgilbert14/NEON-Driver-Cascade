...
| 11 | Driver v2 reintegration | **GATE 0 COMPLETE / PHENOLOGY SEAL 1 IMPLEMENTED AND VERIFIED — SIGNAL CHANGES HELD / NO CANONICAL BYTE CHANGE** | [`docs/DRIVER-V2-SYNTHESIS.md`](DRIVER-V2-SYNTHESIS.md) and the machine-readable compatibility ledger measure all nine current release families without executing sibling code; [`docs/PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the effect-blind contract; Seal 1 adds only the Driver-owned pure two-clock adapter, response-only interval model, one-way values-free climate mask, 38 synthetic fixture families, and an exact Ubuntu/R/package/BLAS runtime lock | nine packages remain reconciled; Seal 1 passed 38 fixtures across six fresh processes with byte-identical output, independent adversarial review, effect/climate isolation, exact Driver-calendar topology, and generated-artifact nonmutation; neither the current nor legacy Phenology RDS family was deserialized | no blind repin and no newly eligible vote; both legacy green-up screens remain HOLD for v2 re-authorization, exact current v2 compatibility/model support remains `UNMEASURED`, and every effect remains sealed | next: run Seal 2 only against exact legacy pin `81e339e9...` and prove registered parity without inspecting current response/effect output; only a separately reviewed successful Seal 2 may advance to Seal 3A current response support; acquire discharge evidence without building an app |
...
|---|---|---|---|---|---|
| Driver baseline | temperature -> green-up | HOLD FOR V2 RE-AUTHORIZATION / PHENOLOGY SEAL 1 VERIFIED / V1 ARTIFACT UNCHANGED | the published annual-temperature family still contains 18 eligible sites and 15/18 direction matches; the spring screen contains 8/18, but both were built from the legacy Phenology pin; the effect-blind [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the current-release response/effect contract, and its pure Seal-1 implementation plus exact `survival 3.8-9` / `metafor 5.0-1` runtime lock passed 38 synthetic fixtures across six clean processes without accessing a current or legacy response family | preserve the exact v1 lineage; create no new vote or canonical byte; keep current compatibility/model support `UNMEASURED` and all effects sealed | prove sections 8.3 and 9.3 only against exact legacy pin `81e339e9...` in Seal 2; do not inspect current response/effect output to repair parity, and do not begin Seal 3 until the separately reviewed parity gate passes |
...
| Plant Phenology Explorer | plant-year green-up onset, leaf-active duration, and green-up coverage/support | HOLD / SEAL 1 VERIFIED / NO BYTE CHANGE | the previously disclosed current release contributes 346/346 direct Driver-calendar matches across 45 sites, with 39 sites at >=6 app-supported years; its data tree differs from the legacy Driver pin; Seal 1 implements the frozen registry as three pure, effect-locked modules and verifies 38 synthetic fixture families, exact calendar topology, deterministic replay, and artifact nonmutation, while current v2 compatibility/model support remains `UNMEASURED` | keep v1 artifacts unchanged; retain source-clock `thin_greenup` only as leaf-active context; use the independent date-clock interval response for v2; Seal 1 accessed neither current/legacy Phenology RDS values nor an effect path | next: exact legacy-pin parity only in Seal 2 under [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md); current response-support access remains forbidden until a separately reviewed Seal 2 passes |
