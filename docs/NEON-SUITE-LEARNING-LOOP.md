...
| 0A | Suite release-health preflight | IN PROGRESS — 3/3 STARTUPS RESTORED; PASSES 1–7 RELEASE-SAFE | 2026-07-18 baseline found drift in all nine companion manifests, moving release inputs, five apps with no executable tests, and Startup Error states in Small Mammal, Phenology, and Ground Beetle. Pass-specific releases now give Small Mammal, Phenology, Plant Diversity, Vegetation, Ground Beetle, Mosquito, and Birds pinned validation, restricted refresh candidates, semantic health, exact manifests, and app-local governance. Birds scientific/runtime authority `97c3e4c` passed its 47-site opportunity/index/manifest contracts, exact-head review, merged-main validation, Pages, and content-aware production smoke | revamp plan + seven published app-local packages; Vegetation docs authority `3391e70` / receipt `da466ea`; Ground Beetle release merge `89caa435`; Mosquito science/runtime receipt `91b4c71`, compact Pages authority `ec0f2ba`, and cover receipt `6450f01`; Birds runtime `97c3e4c` / docs authority `07c852c` | `NONE` (suite-platform); reuse the validated release shape in later passes, but keep product science app-specific | All three baseline startup errors are closed; passes 1–7 have content-aware production receipts; Water Chemistry and Inverts retain startup-only baseline evidence until their passes |
...
Passes 1–7 are complete and production-verified. Phenology Living Poster merge
...
Driver bytes remain unchanged. Water Chemistry Pass 8 is the next one-app cycle.
