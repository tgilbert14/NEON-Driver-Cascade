...
Driver baseline
  ...
  -> Phenology Seal 2 [LEGACY SCHEMA HOLD; PARITY NOT ATTEMPTED]
  -> separately reviewed schema decision [CURRENT RELEASE + SEAL 3 SEALED]
  -> full suite cross-link/deployment audit
```

The formal pass-10 decision is now **DEFER every new app build / AUTHORIZE evidence
acquisition only**. Acquire a pinned discharge bundle first and a pinned herbaceous
clip-harvest bundle second; reopen BUILD only from the registered common-support
floors in `docs/COMPLEMENTARY-APP-GAP-AUDIT.md`. Litterfall remains separate
descriptive forest context. No catalog roster or domain proxy is an eligible join.
...
| 10 | Complementary-product decision | **COMPLETE — DEFER BUILD / ACQUIRE EVIDENCE ONLY** | 2026-07-18 catalog sweep reconciled on 2026-08-04 against all nine released packages and the exact current Driver artifact; no candidate source bundle was present locally | [`docs/COMPLEMENTARY-APP-GAP-AUDIT.md`](COMPLEMENTARY-APP-GAP-AUDIT.md) records measured response-side support, withdraws asserted overlap, and sets exact reopening floors | discharge feasibility first; clip-harvest feasibility second; litterfall descriptive only; no new app, adapter, prior, vote, or Driver byte authorized | documentation-only decision; canonical Driver hashes unchanged |
| 11 | Driver v2 reintegration | **GATE 0 COMPLETE / PHENOLOGY SEAL 1 PUBLISHED / SEAL 2 HOLD AT LEGACY SCHEMA GATE — SIGNAL CHANGES HELD / NO CANONICAL BYTE CHANGE** | [`docs/DRIVER-V2-SYNTHESIS.md`](DRIVER-V2-SYNTHESIS.md) and the machine-readable compatibility ledger measure all nine current release families without executing sibling code; [`docs/PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md) freezes the effect-blind contract; Seal 1 adds only the Driver-owned pure two-clock adapter, response-only interval model, one-way values-free climate mask, 38 synthetic fixture families, and an exact Ubuntu/R/package/BLAS runtime lock; PR #55 head `35c2ca7` merged as `6cb1e9e8`, and receipt PR #56 head `a26428a` merged as `c952687` | receipt PR run `30979039907`, post-merge run `30979297006`, and Pages run `30979295789` passed; the first isolated Seal-2 gate acquired exact legacy commit `81e339e9ed6f34d3d04ca45a7030fea51c4147a5` / `data/sites` tree `30abe869b0f78931929c21e544ffc85ec2238e35` alone and schema-scanned 46 bundles: 45 had nonempty `trend`, while one had a typed non-`NULL` required-column `trend` with zero rows; the frozen registry raised `empty_required_table`, so formal parity was **NOT ATTEMPTED**; a successful manual one-bundle nonempty diagnostic is not parity evidence | no blind repin and no newly eligible vote; Seal 2 is `HOLD`, both legacy green-up screens remain HOLD for v2 re-authorization, current v2 compatibility/model support remains `UNMEASURED`, Driver impact is `NONE`, and Seal 3 plus every v2 effect remains sealed; the isolated gate recorded `current_fetched`, `current_deserialized`, `effect_module_sourced`, and `effect_function_called` all false, while the separate canonical rebuild continued to use recorded legacy v1 pins | Pages build `92220078640`, deploy `92220171068`, and status `92220171073` published exact receipt merge `c952687`; the public root returned HTTP 200; next: publish the Seal-2 HOLD, and treat any legacy-only schema amendment as a new separately reviewed registry stage, never a silent normalization |
...
|---|---|---|---|---|---|
| Driver baseline | temperature -> green-up | HOLD FOR V2 RE-AUTHORIZATION / PHENOLOGY SEAL 2 SCHEMA HOLD / PARITY NOT ATTEMPTED / V1 ARTIFACT UNCHANGED | the published annual-temperature family still contains 18 eligible sites and 15/18 direction matches; the spring screen contains 8/18, but both were built from the legacy Phenology pin; Seal 1 is published and its receipt is merged as `c952687`; the first isolated Seal-2 gate acquired exact legacy pin `81e339e9ed6f34d3d04ca45a7030fea51c4147a5` / tree `30abe869b0f78931929c21e544ffc85ec2238e35` alone and schema-scanned 46 bundles, finding 45 nonempty `trend` tables and one typed non-`NULL` required-column table with zero rows; the frozen registry raised `empty_required_table`, so formal sections 8.3/9.3 parity was not attempted; one manually adapted nonempty bundle is diagnostic only, not parity evidence; the isolated gate's current-fetch/current-deserialization/effect-source/effect-call flags were all false, while the separate canonical rebuild used recorded legacy v1 pins | preserve the exact v1 lineage; create no new vote or canonical byte; keep current compatibility/model support `UNMEASURED`, Driver impact `NONE`, and all v2 effects plus Seal 3 sealed | publish the Seal-2 HOLD; if a legacy-only schema amendment is warranted, introduce it as a new separately reviewed registry stage and never as silent normalization of the frozen contract |
...
| Driver baseline | aquatic climate bridge | DEFER BUILD / ACQUIRE EVIDENCE | released Water and Inverts have zero exact terrestrial Driver site-code matches; domain-year proxies are descriptive diagnostics, not integration keys | no Driver artifact, adapter, prior, or vote | acquire exact Continuous Discharge support and reopen only if discharge x Inverts clears >=3 recorded streams x >=6 common eligible years |
...
| Complementary-app gap audit | streamflow / discharge `DP4.00130.001` as the aquatic hinge | DEFER BUILD / ACQUIRE EVIDENCE FIRST | released Inverts contains 210 density-eligible stream site-years across 24 sites, 23 with >=6 years; candidate-side discharge support and exact overlap remain `UNMEASURED`, so the prior 24–28-site overlap assertion is withdrawn | no `ann_flow()`, prior, app, or vote at Gate 0 | acquire a pinned discharge bundle and reopen only at >=3 recorded streams x >=6 common QC-cleared discharge-and-Inverts years |
...
| Plant Phenology Explorer | plant-year green-up onset, leaf-active duration, and green-up coverage/support | HOLD / SEAL 2 SCHEMA GATE STOPPED / PARITY NOT ATTEMPTED / NO BYTE CHANGE | the previously disclosed current release contributes 346/346 direct Driver-calendar matches across 45 sites, with 39 sites at >=6 app-supported years; its data tree differs from the legacy Driver pin; Seal 1 and receipt PR #56 are published, but the first isolated Seal-2 gate stopped before parity after exact legacy pin `81e339e9...` / tree `30abe869...` produced 45 nonempty `trend` tables and one typed non-`NULL` required-column table with zero rows across 46 schema-scanned bundles; the frozen registry raised `empty_required_table`; a successful manual adaptation of one nonempty bundle is not parity evidence, and current v2 compatibility/model support remains `UNMEASURED` | keep v1 artifacts unchanged; retain source-clock `thin_greenup` only as leaf-active context; use the independent date-clock interval response for v2; the isolated Seal-2 gate fetched or deserialized no current family and sourced or called no effect module/function; scope that claim to the isolated gate because the separate unchanged v1 rebuild used recorded legacy pins for canonical reproduction | publish the HOLD under [`PHENOLOGY-V2-ADAPTER-SPEC.md`](PHENOLOGY-V2-ADAPTER-SPEC.md); any legacy-only schema amendment must be a new separately reviewed registry stage, never silent normalization; Seal 3 remains sealed |
...
sealed. The next action is to publish this HOLD; any legacy-only schema amendment
must be a new separately reviewed registry stage, never a silent normalization.
Discharge feasibility evidence may still be acquired without building a new app.
