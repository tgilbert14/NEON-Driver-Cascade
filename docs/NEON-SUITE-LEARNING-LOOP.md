...
Driver baseline
  ...
  -> Driver v2 integration gate [NO BLIND REPIN; PHENOLOGY ADAPTER NEXT]
...
| 11 | Driver v2 reintegration | **GATE 0 COMPLETE — SIGNAL CHANGES HELD / NO CANONICAL BYTE CHANGE** | [`docs/DRIVER-V2-SYNTHESIS.md`](DRIVER-V2-SYNTHESIS.md) and the machine-readable compatibility ledger measure all nine current release families without executing sibling code; only Mammal is byte-current at Driver-consumed scope | nine packages reconciled; two decision axes separate reusable-contract adoption from ecological-signal adoption | no blind repin and no newly eligible vote; both legacy green-up screens are HOLD for v2 re-authorization | next: register and independently test the current-release Phenology adapter before examining v2 effects; acquire discharge evidence without building an app |
...
|---|---|---|---|---|---|
| Driver baseline | temperature -> green-up | HOLD FOR V2 RE-AUTHORIZATION / V1 ARTIFACT UNCHANGED | the published annual-temperature family still contains 18 eligible sites and 15/18 direction matches; the spring screen contains 8/18, but both were built from the legacy Phenology pin | preserve the exact v1 lineage; create no new vote or canonical byte at Gate 0 | specify and independently test the current 7-day Phenology adapter, interval-censoring policy, registered model, and old/new parity before examining v2 effects |
...
| Plant Phenology Explorer | plant-year green-up onset, leaf-active duration, and green-up coverage/support | HOLD / NO BYTE CHANGE | the current 7-day release contributes 346/346 direct Driver-calendar matches across 45 sites, with 39 sites at >=6 years; its data tree differs from the legacy Driver pin | keep v1 artifacts unchanged; retain coverage as support, never timing | next implementation: pre-specify and independently test the current-release adapter, censoring policy, registered temperature/onset model, desert alternative, and old/new parity before examining v2 effects |
...
0. The next implementation step is the independently specified current-release
Phenology adapter, while discharge feasibility evidence is acquired without
building a new app.
