...
Driver baseline
  ...
  -> Discharge F0 [CANDIDATE FROZEN FOR REVIEW; F1 UNAUTHORIZED]
  -> full suite cross-link/deployment audit
```

The formal pass-10 decision is now **DEFER every new app build / AUTHORIZE staged
feasibility evidence only**. Current authority stops at the Discharge F0
candidate; F1 cannot acquire its manifest/schema inventory until a follow-up
receipt proves the exact reviewed F0 merge and Pages publication. Discharge
remains first and herbaceous clip harvest second; reopen independent review only
...
| Complementary-app gap audit | streamflow / discharge `DP4.00130.001` as the aquatic hinge | DEFER BUILD / GATE F0 CANDIDATE FROZEN FOR REVIEW | exact Inverts `ff23e994` / `data/sites` tree `080534ca` is represented only by committed ledger blob `c2aefd1a...` / SHA-256 `79bb4591...`: 210 density-eligible UTC-year stream cells across 24 sites, 23 with >=6 years; F0/F1/F2 never fetch or deserialize its RDS blobs; discharge support remains `UNMEASURED`; F0 freezes total response-anchored `FALSE`, `historical_uncorrected_1_min_only`, ordinary/BIGC chronology, pre-QC TOMB rejection, `namedLocation` identity, and `REOPEN_REVIEW`/`HOLD` mappings | no `ann_flow()`, discharge summary, prior, app, effect, vote, or Driver byte | publish F0 plus its follow-up exact merge/Pages receipt before F1; then inventory only, followed by separately reviewed F2 payload measurement; reopen review—not build—only at >=3 unambiguous exact streams x >=6 common primary years |
...
leaving Phenology held is valid. The Discharge F0 candidate may proceed through
review and publication without building a new app; no F1 acquisition may begin
until the follow-up exact merge/Pages receipt is merged and verified.
