...
Current cover/default publication identity is in the dated 2026-08-06 register
below; the following paragraphs retain earlier pass-closeout chronology and
science/data authorities.
...
then-current deployed-revision receipt `53991b6f460a97a4abfcee9f62e94cd77c167f89`.
...
- Use **Suite Living Poster V1** on both Pages and the in-app first-run surface:
  ...
  editorial artwork with meaningful alt text and durable provenance, and a compact
  ...
  limitation. Do not spend visible poster space on an illustration disclaimer;
  preserve art status in alt text and provenance while keeping real scientific
  claim limits visible.
...
| 5 | Ground Beetle Tracker | PASS 5 COMPLETE / PRODUCTION VERIFIED | opportunity-complete data PR #14 head `c3dec8d` merged as `4e628f88`; final static-poster PR #15 head `a7950294` merged as cover/runtime authority `89caa435`. Scientific/data authority remains `a615d6c`; at Pass-5 closeout the default/docs authority was `1a768b4`, and the dated 2026-08-06 register records later cover, lifecycle-runtime, and default/docs identities. Exact R 4.5.2 / 91-package / 112-runtime-file manifest SHA-256 `b3da3599f7601fabd697d592897f2238af02315c286ef829d1ab0715815a9266`; effort, zero-catch, taxonomy, detection, QC, cold-bundle, index, handler, cover, and manifest contracts passed | opportunity-complete field-effort grain; individualID expert reconciliation with count conservation; sampled-zero versus invalid/missing effort; detection-frequency and activity-density boundaries; static moss/copper screenprint provenance; identity domains recorded separately | `CONTEXT / HOLD DRIVER INGESTION / NO DRIVER BYTE CHANGE`; Gate-0 measured 388/388 supported calendar matches across 46 sites, 43 at ≥6 years, but no independent adapter, registered mechanism, or ecological vote is authorized | exact final release passed CI `30023971987`, Pages `30023970965`, and content-aware Connect/Pages smoke `30023973253`; Pass-5 authorities retain the production contract and the live first-run/representative-site flow |
...
|---|---|---|---|---|---|
| Mosquito Pulse | mosquito CO2-trap effort denominator (`effort_week`) | `ADOPT` CONTRACT / `CONTEXT` ECOLOGY / NO CANONICAL BYTE CHANGE IN THIS SOURCE PASS | the mosquito release merged upstream as `fdb9aa1` (2026-08-05) replaced the weekly effort column `trap_nights` with `effort_days`; both are `trapHours / 24`, but the new column sums only rows the source marks `valid_effort` (occurred, usable duration, usable identity) rather than every row with positive trap hours, so the refreshed denominator is a QC-screened subset and `mosq_activity` (catch / trap-nights) can rise where unusable deployments existed. Driver scheduled refresh run `31298345511` (2026-08-09) failed closed at `ABBY mosquito effort table lacks required field(s): trap_nights`; every mosquito site carries the new schema, ABBY is only alphabetically first. `validate-artifacts` and `publish` skipped, canonical cascade/search/meta SHA-256 stayed `47b98e48` / `a11a072d` / `00120c52`, and both live surfaces kept serving the last good family | read whichever effort field the bundle publishes, preferring `effort_days` exactly as the producing app does, and fail closed when neither exists; carry the QC-screened basis onto the published `mosq_trap_nights` codebook note so the caveat travels with the number | the moved values land only through a reviewed refresh candidate, not this source pass; mosquito rows remain descriptive context and enter no pooled vote, so no inferential claim changes; preview the deltas with a `publish=false` dry run before any scheduled publication |
...
| My Little Inverts governance/tooling | identity-bound knowledge package, authority separation, and exact production-browser gate | NONE / NO DRIVER BYTE CHANGE (suite-platform) | source `1b059cb` produced direct-child candidate `ecbb23c`; PR #7 check `30894827652` and governance merge `6972817` passed. Pages `30896544721`, Connect #18, and production `30896548595` passed exact live bytes, responsive/focus checks, and a Shiny round trip. Runtime payload remained `87900f675a1e…`; only the Pages/governance identity moved to `sha256:e1d3f1be…`. BUILD-only PR #8 merge `53991b6`, Pages `30900109522`, Connect #19, and production `30900110643` then passed with that identity unchanged | preserve `ff23e994` as the science/data/runtime pin and `6972817` as governance/tooling and identity authority; treat `53991b6` only as the pre-cleanup deployed-revision receipt and do not infer a Driver signal from a repository/publication identity change | reuse the identity-domain, exact-browser, and identity-excluded append-only receipt gates in later suite releases |
| Suite Living Poster | concise companion first impression with executable absence checks | NONE / NO DRIVER BYTE CHANGE (suite-platform) | Policy commit `3889a16` removes cover-visible ornamental illustration disclaimers while retaining meaningful alt text, durable provenance, accessibility, and scientific claim limits. The dated cleanup register below separates each app's current cover/default publication from its scientific/data/runtime authority. All eight removals are terminal and Water is an inspected no-op. A final nine-app production sweep passed 27 responsive Pages cases and nine Connect desktop cases without a retired label, visible `figcaption`, overflow, broken image, or host/Shiny error. | no Driver artifact or ecological change; keep full method and claim detail inside the product and durable science package while the cover preserves only the load-bearing scientific limit in a compact disclosure | require one poster, one CTA, one Driver route, and one compact honesty disclosure; prohibit metric/method/truth layers, duplicate bridges/directories, and cover-visible art disclaimers; visually verify artwork-first narrow order and first-viewport CTA visibility where geometry permits |
...
### 2026-08-06 cover-cleanup authority register

This UI/publication tranche creates no ecological evidence and does not silently
repin any app's science or data. Detailed immutable receipts remain app-local and
in the Driver handoff.

| App | Scientific/data/runtime authority retained | Current cover/default publication authority | Terminal evidence |
|---|---|---|---|
| Small Mammal Tracker | `c4c46fce` | PR #92 head `c1b667b6` -> `fa982438` | exact-head `31067930858`; main `31068784690`; Pages `31068784339`; production `31068784703` |
| Vegetation Structure Explorer | runtime/science `d566b30`; docs `3391e70`; prior receipt `da466ea` | PR #15 head `343e072a` -> `fa083293` | exact-head `31068003862`; main `31069040690`; Pages `31069039945`; central signed-in suite receipt binds Connect content `019ee110-8fd3-abae-aee3-02ea8e4274c8` to exact `fa083293` under R 4.5.2 / 91 packages, followed by live QA |
| Plant Phenology Explorer | science/data `7d0f29f`; prior poster provenance `50106f20` | PR #11 head `36e6725e` -> `c704a377` | exact-head `31066206050`; main `31066622018`; Pages `31066621547`; production `31066622073` |
| Plant Diversity | source-data `a060ee6`; science/runtime `8fc0824`; docs `7292465`; governance `28fab5b`; prior poster `dfb44231` | PR #17 head `f8038f66` -> `7fa45bbe` | exact-head `31066213366`; main `31066454741`; Pages `31066453987`; production `31066454742` |
| Ground Beetle Tracker | science/data `a615d6c`; cover `1be09b7`; lifecycle runtime `a5d3e0e` | docs/default/deployed PR #21 head `fcd01c85` -> `0594cfa` | cover, lifecycle, and docs chains passed; current main `31071862375`; Pages `31071861854`; production `31071862357`; Connect #76 exact `0594cfa` |
| Mosquito Pulse | science/data/runtime `935420e`; science receipt `91b4c71`; prior compact Pages `ec0f2ba`; prior cover receipt `6450f01` | PR #11 head `105a7dba` -> `fdb9aa1a` | exact-head `31067295881`; main `31069571068`; Pages `31069570315`; production `31069571090`; central signed-in suite receipt binds Connect content `019ef0b1-0099-c999-1edc-4d47826044cc` to exact `fdb9aa1a` under R 4.5.2 / 91 packages, followed by live QA |
| Breeding Birds | science/runtime `97c3e4c`; prior docs/default `07c852c` | source `44e7cf92`; direct-child candidate/PR #5 head `a7b954a5` -> current default `bb18be35` | refresh `31067221525` / artifact `8960794005`; exact-head `31108947955`; main `31110952060`; recovery Pages `31113290899`; production `31110952345` attempt 2; Connect #47 / history `019fd779-48d4-3ca1-31f5-e016c02ef2c1` exact `bb18be35`; release `sha256:043aa41d80a5b714b76e0d9bdc035fa636dc87a17200321695b9d9db706502cc` |
...
scientific/data/runtime authorities.
...
green validation, Pages, and semantic production checks. Phenology then-current release
...
cover/runtime authority `89caa435`, then-current audited data authority `a615d6c`, and
then-current default/docs authority `1a768b4` remain distinct; its opportunity-complete
...
then-current deployed receipt `53991b6`; Pages `30900109522`, Connect #19
