...
Discharge feasibility Gate F0 is now **passed**, and its append-only publication
receipt is published. Its
...
`31024946671`, and public HTTP 200 passed on that merge. Receipt PR #60 exact
head `8954c1f8934356c1c1c41706c35cc42680a8b027` passed run `31026452964`,
merged as `b75996a85809ed0cd8ba89121e0de18e22063cc7`, and passed merged-master
run `31027151110` and Pages run `31027144433`; the public root returned HTTP 200
...
merged as then-current deployed receipt
...
The 2026-08-06 cover-label cleanup is a separate UI/publication tranche, not a
new scientific/data release. Current cover/default merges are Small Mammal
`fa982438`, Vegetation `fa083293`, Phenology `c704a377`, Plant Diversity
`7fa45bbe`, Ground Beetle `0594cfa` (after cover `1be09b7` and lifecycle runtime
`a5d3e0e`), Mosquito `fdb9aa1`, Breeding Birds `bb18be35`, and My Little Inverts
`98c44ec`. Water required no cover change; `7feb49e` is release-platform-only.
Birds exact-head/default validation, recovery Pages, exact production parity,
and Connect #47 are terminal success. A final nine-app public sweep passed 27
responsive Pages cases and nine Connect desktop cases. These identities do not
replace the app-specific scientific/data authorities recorded in the suite
register, and no Driver artifact or ecological disposition changed.
...
proved that identity unchanged on the then-current deployed revision. Its release
...
- At baseline, only the Driver repository had `AGENTS.md` and a durable
  ...
  PR #8 merged as pre-cleanup deployed receipt `53991b6` and passed Pages/Connect/live-
...
stops at the F1 values-free file inventory and reviewed non-observation schema
metadata.

1. Run only the authorized Continuous Discharge `DP4.00130.001` F1 file and
   non-observation schema-metadata inventory, then publish and independently
   review its exact receipt before separately authorizing any F2 payload
   measurement. Reopen independent review—not an
...
- The core `29c0ed1` release retained the information-heavy baseline. Owner
  direction on 2026-07-22 produced the Suite Living Poster V1 refresh: hook “Read
  ...
  compact honesty/source footer, and the same frame in-app. PR #5 head `c1bf55d`
  passed run `29963446292` and merged as `50106f20`; main validation
  `29964265558`, Pages `29964264658`, and production health `29964265433` passed.
  The cover changed no science or Driver byte.
...
- Owner direction on 2026-07-22 produced the Suite Living Poster V1 refresh: hook
  “How much can one square hold?”, promise “Explore plant communities from one
  square metre outward.”, CTA “Pick a place”, one Driver route, responsive
  byte-mirrored nested-quadrat art, a compact honesty/source footer, and the same
  frame in-app. PR #11 head `3b9137e` passed run `29963358476` and merged as
  `dfb44231`; main validation `29963570916`, Pages `29963570413`, and production
  health `29963570919` passed. It changed no estimator, source family, bundle, or
  Driver byte.
...
- Driver disposition: `HOLD / CONTEXT ONLY / NO DRIVER DATA BYTE CHANGE`.
  ...
  identity authority `6972817`, and then-current deployed receipt `53991b6`; the latter
...
`a7950294` merged as cover/runtime authority `89caa435`. Scientific/data authority
remains `a615d6c`. At Pass-5 closeout the default/docs authority was `1a768b4`;
the dated 2026-08-06 register records later cover, lifecycle-runtime, and
default/docs identities. Gate-0 synthesis keeps those identity domains separate.
...
- The released family has 46 bundles, 100,163 rows, 33,012 independent field-effort
  ...
  represented before outcomes; duplicate, conflicting, missing, and all-zero
  cases have adversarial fixtures.
- Expert determinations are reconciled at `individualID`, preserve sorting totals,
  and retain species-rank, introduced-context, small-series, and permutation-null
  protections.
- The exact R 4.5.2 / 91-package / 112-runtime-file manifest has SHA-256
  ...
  Cold-bundle, index, handler, cover, and exact-manifest contracts passed.
- CI `30023971987`, Pages `30023970965`, and content-aware production health
  `30023973253` passed on exact merge `89caa435`. Live Pages desktop/390/320 and
  Connect first-run/representative site QA passed without a first-party error.
- Pages and Connect share one static editorial forest-floor screenprint showing a
...
- Governance Pages run `30896544721` published exact index/art/social bytes, including
...
- Final BUILD-only PR #8 is deliberately outside the Pages-payload domain v2.
  Exact head `6c01244c` passed run `30898431839` and merged as the pre-cleanup deployed
