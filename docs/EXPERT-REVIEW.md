...
- **Presentation — richness is composition, and the app should keep saying veg basal area is the productivity floor.** `plant_richness` can *invert* in drylands as a few species dominate wet years (which is exactly why `plant_richness→mammal_cpue` correctly pools to null). The app already carries `veg_ba_ha` as the slow standing-stock floor (0.4 m²/ha at JORN to 56.3 at WREF, `DATA-TAKEAWAYS.md:16`). Keep that framing prominent — it is the honest answer to "richness isn't productivity."
